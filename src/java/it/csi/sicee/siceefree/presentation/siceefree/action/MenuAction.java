...
public class MenuAction extends BaseAction<GlobalMenuModel> implements ModelDriven<GlobalMenuModel> {
	...
	public Class modelClass() {
		return it.csi.sicee.siceefree.dto.GlobalMenuModel.class;
	}
	 ...
	 * @return SUCCESS result.
	 */
	public String goToMnuRicercaCertificatore() throws CommandExecutionException {

		// gestione degli eventi di tipo CLICKED
		ICommand action = initCommand(null, "global", "mnuRicercaCertificatore", "CLICKED");
		String result = action.doCommand(this);
		if (result != null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[MenuAction::goToMnuRicercaCertificatore] returning result [" + result + "]");
			}
			setActiveMenu("mnuRicercaCertificatore");
			return result;
		} else {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[MenuAction::goToMnuRicercaCertificatore] returning default result [SUCCESS]");
			}
			setActiveMenu("mnuRicercaCertificatore");
			return SUCCESS;
		}
